...
| # | 做什么 | 验收 | 预估 |
|---|---|---|---|
| **1** | **T0a** 阈值单一真相源 | 改 `.env` 阈值后刷新页面，前端配色随之变，无需改前端代码 | 1h |
| **2** | **T0b** 删可答性判断副本 | `chat_service.py` 不再出现 `ANSWERABLE_MIN_RELEVANCE` | 0.5h |
| **3** | **T0c** I/O 边缘化 | 上传 10MB 文件期间，`/health` 仍能 1 秒内返回 | 2h |
| **4** | **T0d** 架构约束测试 | 故意在 `api/` 加 `from rag import ...`，测试必须失败 | 1h |
| **5** | **T0e** 文档更正 | ADR-004 阶段 1、本清单 | 0.5h |
| — | **← 停下来 review** | | |
