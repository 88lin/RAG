...
回答要求：
1. 直接用自然语言回答，不要使用 JSON 格式
2. 只在陈述具体事实、数据、规定时标注来源，标记紧跟在该内容之后
3. 以下情况不要标注：过渡句、归纳总结、常识性说明、你自己的建议
4. 同一处内容有多个文档支持时写 [doc_1][doc_2]
5. 不要为了显得严谨而给每句话都加标记 —— 标记过密会掩盖真正的证据来源
6. 可以使用 Markdown 组织回答（标题、列表、表格、加粗）

示例格式：
员工每周五可携带宠物来办公室[doc_1]，宠物需性格温顺并已接种疫苗[doc_2]。
建议提前与同组同事沟通，避免影响对宠物过敏的同事。
